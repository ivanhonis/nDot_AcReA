...
class TradeOrderBook:
    ...
    def buy(self, symbol, slot):
        ...
        if not self.qty_rise_flag and self._slot_position[slot]['qty'] <= self.slot[slot]['max_qty']:
            ...
            if self._slot_position[slot]['qty'] > 0 and self._slot_position[slot]['last_buy_price'] < ask_price_fixed:
                return
    ...
    async def asyc_websocket_bookticker_detect(self):
        ...
        async with self.ts as tscm:
            while True:
                ...
                self.smoot_slow_price_history = savgol_filter(self.renko_slow_price_history, 100, 1)
        ...
        async with self.ts as tscm:
            while True:
                ...
                if self._slot_position[slot]['qty'] > 0:
                    ...
                    if self._slot_position[slot]['take_price'] <= selected_price:
