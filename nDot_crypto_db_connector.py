...
class nDot_db_connector:
    ...
    def __init__(self):
        ...
        self.orderbook_data_store = {}
        self.orderbook_data = {}
        self.orderbook_actual_file = ""
        self.orderbook_files_in_memory = []
        self.orderbook_max_files_in_memory = 2
        ...
        # függően 1 illetve 2 óra lesz az eltérés
    ...
    def load_orderbook_db(self, symbol, day_str):
        ...
        if work_file in self.orderbook_data_store:
            if self.orderbook_actual_file != work_file:
                self.orderbook_data = self.orderbook_data_store[work_file]
                self.orderbook_actual_file = work_file
        else:
            try:
                objectrep = open(work_file, "rb")
                self.orderbook_data_store[work_file] = pickle.load(objectrep)
                self.orderbook_data = self.orderbook_data_store[work_file]
                self.orderbook_actual_file = work_file
                self.orderbook_files_in_memory.append(work_file)
                if len(self.orderbook_files_in_memory) > self.orderbook_max_files_in_memory:
                    del self.orderbook_data_store[self.orderbook_files_in_memory[0]]
                    self.orderbook_files_in_memory = self.orderbook_files_in_memory[1:]
                    gc.collect()
            except:
                self.orderbook_data = {}
                self.orderbook_actual_file = ""
    ...
    def load_tickdata_db(self, symbol, day_str):
        ...
        if work_file in self.tickdata_data_store:
            ...
            try:
                ...
                if len(self.tickdata_files_in_memory) > self.tickdata_max_files_in_memory:
    ...
    def get_tick_data(self, symbol, idt):
        shifter = 0
        if idt < self.dt_sync_winter1:
            shifter = (60 * 60) * 2
        elif self.dt_sync_winter1 <= idt < self.dt_sync_winter2:
            shifter = 999999
            # ezt a tartományt nem tudom megfejteni így itt nem adok választ
...
if __name__ == '__main__':
    ...
    # date_time_string = "2022-06-01 00:00:00"
    # dt = datetime.fromisoformat(date_time_string)
    #
    # maxit = 60 * 60 * 24 * 30 * 5
    #
    ...
    #     print("\r" + f"job ready: {ix} / {maxit}", end="")
    #
    #     result_tick = n_db.get_tick_data("BTCUSDT", dt)
    #     if result_tick and len(result_tick['all']):
    #         avg_prices.append(float(result_tick['avg_price']))
    #
    #     dt = dt + timedelta(seconds=1)
    #
    # avg_prices = np.array(avg_prices)
    # np.save("BTCUSDT_avg_price", avg_prices)

    a = np.load("BTCUSDT_avg_price.npy")
    ...
    for s in range(2, 25):
        ...
        for ix in range(maxit):
            ...
            if profit > .18 / 100:
                case += 1
                # print("\r" + f"case: {ix} / {case}", end="")

        print("")
        print(s, maxit/case / 60)
        print("")
